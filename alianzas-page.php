...
//
$idAlianza = $_GET["idAlianza"];
//
$query = "SELECT * FROM alianzas WHERE al_id = " . $idAlianza;
...
<head>
    ...
    <!-- Meta tags -->
    <meta content="<?php echo $arrContenido["al_nombre"]; ?> | ALIANZAS | APOSGRAN | Asociación Argentina de Poscosecha de Granos" name="title" />
    ...
    <!--Metatags FB-->
    <meta property="og:title" content="<?php echo $arrContenido["al_nombre"]; ?> | ALIANZAS | APOSGRAN | Asociación Argentina de Poscosecha de Granos">
    ...
    <meta name="twitter:title" content="<?php echo $arrContenido["al_nombre"]; ?> | ALIANZAS | APOSGRAN | Asociación Argentina de Poscosecha de Granos">
    ...
    <title><?php echo $arrContenido["al_nombre"]; ?> | ALIANZAS | APOSGRAN | Asociación Argentina de Poscosecha de Granos</title>
...
<body id="top">
    ...
    <header class="header-i">
        ...
        <!-- Slider -->
        <div class="slider-i" style="background-image: url('<?php echo _CONST_DOMINIO_ ?>assets/slider/alianzas.jpg');margin-top:0px">
            <h2>ALIANZAS</h2>
        </div>
    ...
    <main role="main">
        ...
        <!-- Contenido 1 -->
        <section class="contenido-curso cont-seminarios">
            <div class="container">

                <div class="alianzas-g">
                    <div class="col-12">
                        <div class="row">
                            <div class="col-12">
                                <h2><?php echo $arrContenido["al_nombre"]; ?></h2>
                            </div>
                            <div class="col-md-6 col-lg-3 alinzas-card-marg">
                                <div class="alinzas-card">
                                    <?php
                                    $queryAl = "SELECT ia.*
                                    FROM integrantesalianzas ia
                                    LEFT JOIN alianzasxintegrantes ai ON ai.axi_id_integrante = ia.int_id
                                    WHERE ai.axi_id_alianza = " . $idAlianza;
                                    $rsContAl = $objContenido->getAllContenido($link, $queryAl);
                                    ?>
                                    <ul>
                                        <?php while ($arrContenidoAl = $rsContAl->fetch(PDO::FETCH_BOTH)) { ?>
                                            <?php if ($arrContenidoAl["int_logo"] == "nd") { ?>
                                                <li><?php echo $arrContenidoAl["int_nombre"]; ?></li>
                                            <?php } else { ?>
                                                <li><img src="<?php echo _CONST_DOMINIO_ ?>assets/alianzas/<?php echo $arrContenidoAl["int_logo"]; ?>" alt="" class="img-fluid"></li>
                                            <?php } ?>
                            ...
                            <div class="col-md-6 col-lg-5">
                                <?php echo html_entity_decode($arrContenido["al_texto"]);  ?>
                            ...
                            $queryAl = "SELECT ia.*
                                    FROM contenido2 ia
                                    LEFT JOIN alianzasxcursos ai ON ai.axc_id_curso = ia.id
                                    WHERE ai.axc_id_alianza = " . $idAlianza;
                            $rsContAl = $objContenido->getAllContenido($link, $queryAl);
                            $intQtyRecords = $rsContAl->rowCount();
                            ?>
                            <?php if ($intQtyRecords > 0) { ?>
                                <div class="col-md-12 col-lg-3 listacursos">
                                    <div class="alinzas-cursos">
                                        <h3>Cursos</h3>
                                        <?php while ($arrContenidoCl = $rsContAl->fetch(PDO::FETCH_BOTH)) { ?>
                                            <a href="<?php echo _CONST_DOMINIO_ ?>cursos/<?php echo $arrContenidoCl["id"]; ?>/<?php echo buildLink($arrContenidoCl["titulo"]); ?>"><?php echo $arrContenidoCl["titulo"]; ?></a>
                                        <?php } ?>
