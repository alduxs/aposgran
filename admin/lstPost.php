...
$query = "SELECT * FROM contenido2 ORDER BY publicada DESC, home DESC,fecha DESC,id DESC LIMIT ?,?";
			...
			<div class="row wrapper border-bottom white-bg page-heading">
				<div class="col-sm-12">
					...
					<ol class="breadcrumb">
						...
						<li class="active"><strong>Listar cursos</strong></li>
					</ol>
				</div>
			</div>
										...
										<tbody>
											...
											<?php while ($arrContenido = $rsCont->fetch(PDO::FETCH_BOTH)) { ?>
												<tr>
													...
													<td><?php echo categorias($arrContenido["seccion"])[0]; ?></td>
													<td>
														<?php
														$query5 = "SELECT ac.axc_id_alianza, a.al_nombre
																FROM alianzasxcursos ac
																LEFT JOIN alianzas a ON a.al_id = ac.axc_id_alianza
																WHERE ac.axc_id_curso = " . $arrContenido["id"];
														$rsCont5 = $objContenido->getAllContenido($link, $query5);
														?>
														<?php while ($arrCont5 = $rsCont5->fetch(PDO::FETCH_BOTH)) { 
															echo $arrCont5["al_nombre"]." | ";
														} ?>
													...
													<td><?php if ($arrContenido["publicada"] == 0) { ?><a href="#" class="btn btn-default btn-circle"><i class="fa fa-check"></i></a><?php } else { ?><a href="#" class="btn btn-info btn-circle"><i class="fa fa-check"></i></a><?php } ?></td>
													<td><?php if ($arrContenido["home"] == 0) { ?><a href="#" class="btn btn-default btn-circle"><i class="fa fa-check"></i></a><?php } else { ?><a href="#" class="btn btn-info btn-circle"><i class="fa fa-check"></i></a><?php } ?></td>
